...
class UnitBreaker(object):
  ...
  def visualize_units_boundary(gr, gr_smooth, tvd, boundary_flags, start = 0, n_samples = 1000, n_pics = 6):
    """
    Visualizing units boundary on gr curve.
    ...
    gr_smooth : 1D numpy array, shape (n_samples,)
      The smoothed GR curve.

    tvd : 1D numpy array, shape (n_samples,)
      The input tv depth.

    boundary_flags : 1D numpy array, shape (n_samples,)
      Boundary flag of points, 1 if low peak (or just peak), 2 if high peak, 0 otherwise.
  ...
  def visualize_units(gr, labels, boundary_flags, start = 0, n_samples = 1000, n_pics = 6):
    """
    Visualizing units label shape code on gr curve.
